...
## Deployment (GitHub Actions → all-inkl via FTPS)

Every push to `main` triggers [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml),
which deploys over FTPS to all-inkl. Before uploading it appends `?v=<commit-sha>` to the
...
| Secret         | Description                                |
| -------------- | ------------------------------------------ |
| `FTP_SERVER`   | FTP host (e.g. `wXXYYYY.kasserver.com`)    |
...
| ---------------- | ------------------------------------------------------------- |
| `FTP_SERVER_DIR` | Target document root of the subdomain (e.g. `/478breathing.hydrax.org/`) |
